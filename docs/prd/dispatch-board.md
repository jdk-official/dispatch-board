...
- `docs/backlog/pbi/PBI-022-prd-rebaseline.md` (this work item).
...
- **FR-185** The collector shall prune by age only the records of sessions linked to no project whose last activity is older than the session window; every other deletion (a vanished transcript, an excluded session, a removed project) goes through the mass-delete guard (FR-184). *(Row 12: "the last 7 days plus linked sessions, pruned by age only". Amended 2026-09-11 from "shall delete … only" at the PBI-019 spec gate, row Q-3; confirmed by the owner, verbatim: "Q-3 and A-47: yes.")*
