...
let fails = 0;
const check = (label) => {
  for (const n of ['overview', 'usage']) {
    const t = strip(el('panel-' + n).innerHTML), i = t.indexOf('Next limit hit');
    const snip = i < 0 ? '(no forecast tile)' : t.slice(i, i + 110);
    if (i < 0 || !/estimate|no forecast/.test(snip)) fails++;
    console.log(`${label} ${n}: ${snip}`);
  }
};
...
const e2 = el('project'); e2.value = 'other'; (e2.listeners.change || []).forEach(f => f({ target: e2 }));
check('other-sessions');
console.log(`console.error calls: ${errors.length}`); errors.forEach(x => console.log('  ' + x.split('\n')[0]));
console.log(fails || errors.length ? 'AC-80 check: FAIL' : 'AC-80 check: PASS');
