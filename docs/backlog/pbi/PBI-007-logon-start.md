...
- [x] **AC-65** When the owner logs on to Windows with no Claude Code session open, the page at the local server's address shall show the sessions active in the last 7 days within 10 minutes. *(FR-90, FR-91, NFR-17, NFR-21; demonstration.)*
...
- **AC-65 — demonstrated 2026-09-19 by the owner's log-on.** The PC restarted (Windows logged restarts 19:22–19:25 local) and the owner logged on at 19:26:34 local (Winlogon 7001). With no one starting anything, Task Scheduler (parent svchost PID 4256) started `run_local.py --target collector` at 19:26:35 and `--target server` at 19:26:50 under `pythonw.exe`. `out/local/logs/collector.log`: "19:26:43 run_local: starting collector", then the first committed pass at 19:27:01 ("6 sessions … 251 runs, 2 projects, 10 tabs"), 27 s after log-on. `server.log`: "19:26:50 server: serving http://127.0.0.1:8765", then `GET /` and `/api/snapshot` 200. The snapshot served lists 6 sessions, every one active in the last 7 days. Observed at the same time: Task Scheduler started a second collector at 19:28:35 despite `MultipleInstancesPolicy` IgnoreNew; landed as PBI-040 (a single-instance guard). It does not affect AC-65.
...
| BOARD-tidy gate | always | passed 2026-09-19 | AC-65 demonstrated by the owner's log-on; AC-72 deferred to PBI-016 by the owner; Done write, ledger deregistered |
