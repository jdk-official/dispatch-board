...
19. **Agent catalogue tab, built first (owner, 2026-09-11).** The owner's words: "Can we add a PBI for a new tab to show all the agents in the catalogue with coverage of what we are using? Make this the first thing we build." Added to the plan as PBI-017. At the plan gate the owner widened it: "Include skills too" and "Show what each agent is for", so the tab lists the catalogue's agents and skills, grouped by purpose, with full descriptions.
